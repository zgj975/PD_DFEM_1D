...
The PD database 
1. TPdNode*(BBPD, NOSPD, BeamPD...)
2. TPdNodeCollector: The PD nodes collector only include NODE information
3. TPdDataCollector: The PD database include MATERIAL,PROPERTY,BOUNDARY CONDITION etc.1
4. TPdModelBase:	 Some public operations for TPdDataCollector
== == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ==
Date            Name                    Description of Change
2018 / 04 / 05		Zheng Guojun			Create
2019 / 07 / 31		Zheng Guojun			Added TPdNodeBBPD/TPdNodeNOSPD/TPdNodeBeamPD
$HISTORY$
== == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ==
*/
...
namespace DLUT
{
	namespace SAE
	{
		namespace PERIDYNAMIC
		{
			...
			public:
				MATRIX_SINGLE_STIFFNESS_PD&			SK() { return m_single_stiffness; }
				const MATRIX_SINGLE_STIFFNESS_PD&	SK() const { return m_single_stiffness; }
				MATRIX_PD_FORCE_PD&					ForceOfBond() { return m_force; }
				const MATRIX_PD_FORCE_PD&			ForceOfBond() const { return m_force; }
			private:
				int							m_eid_j;				//	Element J
				double						m_volume_index;			//	Modified volume index
				vector<TPdBond>				m_bonds;				//	TPdBonds.
				MATRIX_SINGLE_STIFFNESS_PD	m_single_stiffness;		//	Single stiffness matrix of Bond_ij
				MATRIX_PD_FORCE_PD			m_force;				//	FORCE vector of Element_ij
			};
			...
			public:
				...
				{	
					...
					for (int pid = 0; pid < PartCounts(); ++pid)
					{
						...
						if (stype == "BEAM")
						{							
							double area = section.GetSectionValue("Area");
							for (int eid : eids)
							{
								TPdElement& pd_element = m_pd_meshcore.Element(eid);
								pd_element.Area() = area;
							}
						}
					}
				...
				void				UpdateFamilyInParts()
				{
					...
					//	Secondly, rebuild the family information for every node
					for (const TPart& part : Parts())
					{
						...
						for (int ei : eleIds)
						{
							...
							TCoordinate coor_i = element_i.CoordinateInElement(0);
							...
							for (int ej : ejIds)
							{
								...
								TCoordinate coor_j = element_j.CoordinateInElement(0);
									...
									(abs(coor_i.z() - coor_j.z()) < dis_for_juge))
								{
									...
									{
										Vector3d cij;
										double intersect_volume_index = CalModifiedVolumeIndex(element_i, element_j);
										//	如果单元相交面积不为0，则列为FamilyElement
										if (intersect_volume_index > 0)
										{
											element_i.InsertFamilyElement(ej, intersect_volume_index);
											
											//	取j单元的积分点，与i单元的中心点形成FamilyElement的TPdBond
											TIntegrationPoint& xi = element_i.IP(1);
					...
					parallel_for_each(eids.begin(), eids.end(), [&](int ei)
						{
							...
							const TCoordinate& ei_coord_local = T * element_i.CoordinateInElement(0);
							LIST_NJ_FAMILY_ELEMENT& familyElements = element_i.FamilyElements();

							for (TPdFamilyElement& family_elem : familyElements)
							{
								TPdElement& element_j = m_pd_meshcore.Element(family_elem.Id());
								const TCoordinate& ej_coord_local = T * element_j.CoordinateInElement(0);
			...
			private:
				double				CalModifiedVolumeIndex(TPdElement& element_i, TPdElement& element_j)
				{
					double index_of_j = 0;
					...
					TCoordinate center_i = element_i.CoordinateInElement(0.5);
					TCoordinate center_j = element_j.CoordinateInElement(0.5);

					double r = element_j.SideLength() / 2.0;
					double d = Distance_2pt(center_i, center_j);
