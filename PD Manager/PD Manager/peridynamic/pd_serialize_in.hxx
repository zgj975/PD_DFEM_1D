			...
			public:				
				...
				void		WriteLsdynaFile(string filename, TPdModel& pdModel)
				{
					...
					if (fout != NULL)
					{
						...
						fprintf(fout, "*ELEMENT_BEAM_ORIENTATION\n");
					...
					//		fprintf(fout, "%8d%8d%8d%8d%8d\n", element.Id()+1, element.PartId(), element.NodeId(0)+1, element.NodeId(1)+1, element.NodeId(2)+1);
							fprintf(fout, "%8d%8d%8d%8d\n", element.Id() + 1, element.PartId(), element.NodeId(0) + 1, element.NodeId(1) + 1);
							fprintf(fout, "%10f%10f%10f\n", element.LocalCoorSystem()(1, 0), element.LocalCoorSystem()(1, 1), element.LocalCoorSystem()(1, 2));
						}
			...
			private:
				void		readMeshData(string filename, TPdModel& pdModel)
				{
					...
					while (!fin.eof())
					{
						...
						else if (string("*ELEMENT_BEAM") == line)
						{
							streampos pos = fin.tellg();
							while (getline(fin, line))
							{
						...
						else if (string("*ELEMENT_BEAM_ORIENTATION") == line)
						{
							streampos pos = fin.tellg();
							while (getline(fin, line))
							{
								...
								getline(fin, line);
								Vector3d vec_y;
								vec_y.x() = ParseString<double>(line.substr(0, 10));
								vec_y.y() = ParseString<double>(line.substr(10, 10));
								vec_y.z() = ParseString<double>(line.substr(20, 10));

								pdModel.PdMeshCore().InsertElement(nids, vec_y, eid, pid);
								map_fem_eid_global_to_fem_eid_local.insert(pair<int, int>(eid, pdModel.PdMeshCore().ElementCount() - 1));
				...
				void		readCalculateInfo(string filename, TPdModel& pdModel)
				{
					...
					while (!fin.eof())
					{
						...
						else if (string("*SECTION_BEAM") == line)
						{
							streampos pos = fin.tellg();
							while (getline(fin, line))
							{
								...
								getline(fin, line);
								double Area = ParseString<double>(line.substr(0, 10));
								double ISS = ParseString<double>(line.substr(10, 10));
								double ITT = ParseString<double>(line.substr(20, 10));

								TSection sec(pid);
								sec.Type() = string("BEAM");
								sec.InsertSectionData("Area", Area);
								sec.InsertSectionData("ISS", ISS);
								sec.InsertSectionData("ITT", ITT);
